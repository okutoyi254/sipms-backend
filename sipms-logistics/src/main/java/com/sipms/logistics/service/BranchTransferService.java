package com.sipms.logistics.service;
...
import com.sipms.logistics.enums.*;
...
public class BranchTransferService {
    ...
    public void approveTransfer(Long transferId, String approvedBy) {
        ...
        for (StockTransferItem item : transfer.getTransferItems()) {
            ...
            Integer approvedQty = item.getRequestedQuantity();

            if (approvedQty <= 0) {
                throw new IllegalStateException(
                        "Invalid approved quantity for product " + item.getProduct().getProductName()
                );
            }
